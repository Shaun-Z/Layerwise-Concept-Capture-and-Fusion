...
class TimmFCVWrapper(CopyAttrWrapper):
    """
    ...
    - The start of i-th block's backpropagation:
      feat = block_output  # (bsz, N, D)
      latent_feat = F.normalize(self.norm(feat), dim=-1)  # (bsz, N, D)
    ...
    def dot_concept_vectors(self, concept_vectors: torch.Tensor, power: int = 0):
        ...
        for enum_idx, layer_idx in sorted_indices:
            ...
            if token_grad is not None:
                ...
                current_concept_vectors = F.normalize(token_grad, dim=-1)  # (M, B, N, D)
