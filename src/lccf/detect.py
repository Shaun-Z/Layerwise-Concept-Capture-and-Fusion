...
def detect_and_wrap(model: Any,
    ...
    async_compute: if True, use async wrapper (A*Wrapper) which computes gradients during forward pass
                   This overrides use_grad - A*Wrappers use gradient computation during forward.
    """
    if model is None:
    ...
    if prefer == "openclip" and isinstance(model.visual, OpenCLIPViT):
        if async_compute:
            # Note: AOpenCLIPWrapper not implemented yet, fallback to GradWrapper
    ...
    if prefer == "timm" and isinstance(model, TimmViT):
        if async_compute:
            return ATimmWrapper(model, layer_indices=layer_indices, include_private=include_private)
        elif use_grad:
    ...
    if prefer == "torchvision" and isinstance(model, TorchViT):
        if async_compute:
            return ATorchvisionWrapper(model, layer_indices=layer_indices, include_private=include_private)
        elif use_grad:
